...
export default function Flashcard({ word, onGrade }) {
    ...
    const bgRight = useTransform(x, [0, 150], ["rgba(255, 255, 255, 0)", "rgba(74, 222, 128, 0.2)"]); // Green tint
    const bgLeft = useTransform(x, [-150, 0], ["rgba(244, 63, 94, 0.2)", "rgba(255, 255, 255, 0)"]); // Red tint
        ...
        <div className="max-w-md mx-auto perspective-1000 w-full h-[500px] flex items-center justify-center relative">
            ...
            >
                ...
                >
                    {/* Front */}
                    <motion.div
                        style={{ backgroundColor: x.get() > 0 ? bgRight : bgLeft }}
                        className="absolute w-full h-full backface-hidden bg-white rounded-2xl shadow-xl border border-slate-200 flex flex-col items-center justify-center p-8"
                    >
                        <div className="text-xs font-bold text-indigo-500 uppercase tracking-wider mb-4">German</div>
                        <h2 className="text-4xl md:text-5xl font-bold text-slate-800 text-center mb-6">{word.id}</h2>
                        ...
                        {word.context && (
                            <div className="w-full mt-4 z-10" onPointerDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}>
                                ...
                                {showContext && (
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 w-full animate-in fade-in slide-in-from-bottom-2">
                                        <p className="text-slate-600 italic text-center">"{word.context}"</p>
                                    </div>
                                )}
                            </div>
                        )}

                        <p className="mt-auto text-slate-400 text-sm absolute bottom-8">Tap to flip • Drag to grade</p>
                    </motion.div>

                    {/* Back */}
                    <div className="absolute w-full h-full backface-hidden rotate-y-180 bg-indigo-50 rounded-2xl shadow-xl border border-indigo-100 flex flex-col p-8">
                        ...
                        <div className="grid grid-cols-4 gap-2 mt-6" onPointerDown={(e) => e.stopPropagation()}>
                            <button
