...
export default function BookDetailView() {
    ...
    useEffect(() => {
        ...
        // 1. Fetch Book Data (Real-time listener for updates like background processing)
        const bookRef = doc(db, 'users', currentUser.uid, 'books', id);
        const unsubBook = onSnapshot(bookRef, (docSnapshot) => {
        ...
        // 2. Fetch Progress
        const progressRef = doc(db, 'users', currentUser.uid, 'bookProgress', id);
        const unsubProgress = onSnapshot(progressRef, (docSnapshot) => {
            if (docSnapshot.exists()) {
    ...
    const handleChapterClick = (chapter) => {
        ...
        if (isUnlocked || true) { // Temporarily allowing all for testing
            ...
            // Assuming chapter has an ID or using index/number
            const chapterId = chapter.id || chapter.number;
            navigate(`/book/${id}/read/${chapterId}`);
        } else {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden mb-8">
                ...
                {book.currentProcessingChapter && (
                    <div className="bg-indigo-600 text-white px-8 py-3 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <Loader2 size={20} className="animate-spin" />
                            <span className="font-medium">
                                Processing Chapter {book.currentProcessingChapter} of {book.totalChapters}...
                            </span>
                        </div>
                        <span className="text-indigo-200 text-sm">
                            {Math.round((book.currentProcessingChapter / book.totalChapters) * 100)}% Complete
                        </span>
                    </div>
                )}
                ...
                <div className="p-8">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-slate-800 dark:text-white">Chapters</h3>
                    ...
                    <div className="space-y-3">
                        {book.chapters?.map((chapter, index) => {
                            ...
                            const isOptimizing = book.isAdapted && !chapter.isAdapted;
                            ...
                            return (
                                ...
                                ${isCurrent ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 ring-1 ring-indigo-500' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-300 hover:bg-slate-50 dark:hover:bg-slate-700'}
                                ${isLocked ? 'opacity-50 cursor-not-allowed bg-slate-50 dark:bg-slate-800' : ''}
                                ${isOptimizing ? 'opacity-75 bg-slate-50 dark:bg-slate-800 cursor-wait' : ''}
                            `}
                                >
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold mr-4
                                ${isCompleted ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-300' : isCurrent ? 'bg-indigo-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400'}
                            `}>
                                        {isCompleted ? <CheckCircle size={20} /> : (chapter.number || index + 1)}
                                    </div>

                                    <div className="flex-1">
                                        <h4 className={`font-bold ${isCurrent ? 'text-indigo-900 dark:text-indigo-300' : 'text-slate-800 dark:text-white'}`}>
                                            {chapter.title}
                                            {isOptimizing && (
                                    ...
                                    <div className="text-slate-400 dark:text-slate-500">
                                        {isLocked ? <Lock size={18} /> :
                                            isOptimizing ? <RotateCcw size={18} className="animate-spin" /> :
                                                <PlayCircle size={20} className={isCurrent ? 'text-indigo-600 dark:text-indigo-400' : ''} />}
                                    </div>
                                </div>
                            );
                        })}
